...
async function checkDoctorAvailability(
  ...
  const conflictEnd = new Date(
    conflicting.scheduledAt.getTime() + conflicting.duration * 60 * 1000,
  );
  ...
  try {
    ...
    if (req.query.dateStart || req.query.dateEnd) {
      ...
      if (req.query.dateStart) {
        (whereClause.scheduledAt as Record<string, unknown>).gte = new Date(
          req.query.dateStart as string,
        );
      }
      if (req.query.dateEnd) {
        (whereClause.scheduledAt as Record<string, unknown>).lte = new Date(
...
appointmentRoutes.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (
        !patient ||
        patient.user.tenantId !== tenantId ||
