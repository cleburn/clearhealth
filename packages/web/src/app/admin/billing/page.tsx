...
export default function BillingPage() {
  ...
  const pendingCount = records.filter(
    (r) => r.status === ClaimStatus.PENDING,
  ).length;
  const submittedCount = records.filter(
    (r) => r.status === ClaimStatus.SUBMITTED,
  ).length;
  const approvedCount = records.filter(
    (r) => r.status === ClaimStatus.APPROVED,
  ).length;
  const _deniedCount = records.filter(
    (r) => r.status === ClaimStatus.DENIED,
  ).length;
    ...
    <div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <p className="mt-1 text-gray-600">
          Insurance claims, payments, and financial reporting.
        </p>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          <StatsCard
            icon={DollarSign}
            label="Total Revenue"
            value={report ? `$${report.totalAmount.toLocaleString()}` : "$0"}
          />
