...
const PATIENT_UPDATABLE_FIELDS = ['emergencyContactName', 'emergencyContactPhone', 'phone', 'insuranceId'];
 ...
 */
patientRoutes.get('/', requireRole('DOCTOR', 'ADMIN', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (req.user.role === 'DOCTOR') {
      ...
      if (!doctor) {
        res.status(403).json({ error: 'Doctor profile not found', code: 'FORBIDDEN' });
        return;
      }
 ...
 */
patientRoutes.post('/', requireRole('ADMIN', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
    ...
    if (existingUser) {
      res.status(409).json({ error: 'A user with this email already exists', code: 'CONFLICT' });
      return;
    }
    ...
    logger.info('Patient created', { patientId: result.patient.id, userId: result.user.id });
 ...
 */
patientRoutes.patch('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
    ...
    if (req.user.role === 'PATIENT') {
      ...
      const disallowedFields = requestedFields.filter((f) => !PATIENT_UPDATABLE_FIELDS.includes(f));
      ...
      res.status(403).json({ error: 'Doctors cannot update patient records', code: 'FORBIDDEN' });
      return;
    }
 ...
 */
patientRoutes.delete('/:id', requireRole('ADMIN', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
    ...
    logger.info('Patient soft-deleted', { patientId, userId: patient.userId });
 ...
 */
patientRoutes.get('/:id/history', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
    ...
    logger.error('Get patient history error', { error: (err as Error).message });
    res.status(500).json({ error: 'Internal server error', code: 'INTERNAL_ERROR' });
  }
});
