  ...
  try {
    ...
    if (req.query.dateStart || req.query.dateEnd) {
      ...
      if (req.query.dateStart) {
        (whereClause.createdAt as Record<string, unknown>).gte = new Date(req.query.dateStart as string);
      }
      if (req.query.dateEnd) {
        (whereClause.createdAt as Record<string, unknown>).lte = new Date(req.query.dateEnd as string);
      }
    }
    ...
    logger.error('List billing records error', { error: (err as Error).message });
 ...
 */
billingRoutes.post('/claims', requireRole('ADMIN', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
    ...
    const claimPayload = {
      ...
      serviceDate: billingRecord.appointment.scheduledAt.toISOString().split('T')[0],
 ...
 * Accessible by: ADMIN, SUPER_ADMIN
 */
billingRoutes.post('/claims/:id/followup', requireRole('ADMIN', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
 ...
 * Accessible by: ADMIN, SUPER_ADMIN
 */
billingRoutes.get('/reports', requireRole('ADMIN', 'SUPER_ADMIN'), async (req: AuthenticatedRequest, res: Response) => {
  try {
    const tenantId = req.tenantId;
    if (!tenantId || !req.user) {
      res.status(401).json({ error: 'Authentication required', code: 'AUTH_REQUIRED' });
    ...
    const startDate = queryParsed.data.startDate ? new Date(queryParsed.data.startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const endDate = queryParsed.data.endDate ? new Date(queryParsed.data.endDate) : new Date();
    ...
    for (const status of ['PENDING', 'SUBMITTED', 'APPROVED', 'DENIED', 'PAID']) {
    ...
    const providerMap = new Map<string, { doctorName: string; claimCount: number; totalAmount: number }>();
    for (const record of records) {
      ...
      const existing = providerMap.get(doctorId) || { doctorName, claimCount: 0, totalAmount: 0 };
      existing.claimCount += 1;
      existing.totalAmount += Number(record.amount);
      providerMap.set(doctorId, existing);
    }

    const byProvider = Array.from(providerMap.entries()).map(([doctorId, data]) => ({
      doctorId,
