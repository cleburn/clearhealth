...
describe('RBAC Permission Matrix', () => {
  ...
  describe('SUPER_ADMIN role', () => {
    ...
    it('has global scope on all resources', () => {
      const resources: Resource[] = [
        'patient', 'appointment', 'visit_note', 'billing',
        'insurance', 'user', 'tenant', 'audit_log', 'report',
      ];
  ...
  describe('hasPermission()', () => {
    ...
    it('correctly handles all action types', () => {
      const actions: Action[] = ['create', 'read', 'update', 'delete', 'list', 'export'];
  ...
  describe('getPermissionScope()', () => {
    ...
    it('returns "global" for SUPER_ADMIN on patient resource', () => {
      expect(getPermissionScope(UserRole.SUPER_ADMIN, 'patient')).toBe('global');
    });
  ...
  describe('Permission matrix integrity', () => {
    ...
    it('scope escalates with role hierarchy: own -> assigned -> tenant -> global', () => {
      ...
      const superAdminScope = getPermissionScope(UserRole.SUPER_ADMIN, 'patient');

      expect(scopeOrder.indexOf(patientScope!)).toBeLessThan(scopeOrder.indexOf(doctorScope!));
      expect(scopeOrder.indexOf(doctorScope!)).toBeLessThan(scopeOrder.indexOf(adminScope!));
      expect(scopeOrder.indexOf(adminScope!)).toBeLessThan(scopeOrder.indexOf(superAdminScope!));
    });
